...
from app.service.task import ActionAssignTaskData, ActionTaskStateData
...
class TestWorkforce:
    ...
    def test_eigent_make_sub_tasks_success(self):
        ...
        with (
            ...
            patch.object(
                workforce, "_decompose_task", return_value=mock_subtasks
            ),
            patch(
                "app.utils.workforce.validate_task_content", return_value=True
            ),
        ):
    ...
    async def test_eigent_start_success(self):
        ...
        with (
            patch.object(
                workforce, "start", new_callable=AsyncMock
            ) as mock_start,
    ...
    async def test_find_assignee_with_notifications(self, mock_task_lock):
        ...
        assignments = [
            ...
            TaskAssignment(
                task_id="sub_1", assignee_id="worker_1", dependencies=[]
            ),
            TaskAssignment(
                task_id="sub_2", assignee_id="worker_2", dependencies=["sub_1"]
            ),
        ]
...
class TestWorkforceIntegration:
    ...
    async def test_full_workforce_lifecycle(self):
        ...
        with (
            patch.object(workforce, "_decompose_task", return_value=subtasks),
            patch(
                "app.utils.workforce.validate_task_content", return_value=True
            ),
